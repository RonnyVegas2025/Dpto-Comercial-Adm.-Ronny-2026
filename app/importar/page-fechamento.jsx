...
function parseRow(row) {
  const mesRef = findKey(row, 'Mes Ref.') ?? findKey(row, 'Mês Ref.') ?? findKey(row, 'Mes Ref') ?? findKey(row, 'Mês Ref');
  return {
...
export default function ImportarFechamento() {
  ...
  const handleImport = async () => {
    setStatus('importing');
    try {
      // Busca IDs das empresas pelo produto_id
      ...
      {status === 'confirming' && (
        <div style={s.card}>
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={s.table}>
              <thead>
                <tr>{['ID','Empresa','Mês Ref.','Vendas','Taxa'].map(h =>
            ...
            {preview.length > 15 && <div style={{ textAlign:'center', padding:12, color:'#6b7280', fontSize:'0.8rem' }}>... e mais {preview.length - 15} registros</div>}
          </div>
        </div>
      )}
