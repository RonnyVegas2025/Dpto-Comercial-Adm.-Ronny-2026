...
export default function ImportarEmpresas() {
  ...
  // Carrega a biblioteca xlsx apenas no navegador
  useEffect(() => {
  ...
  const handleFile = useCallback((f) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        const raw = xlsxLib.utils.sheet_to_json(ws, { raw: false });
        const parsed = raw.map(parseRow).filter(r => r.nome && r.produto_id);
        setPreview(parsed);
        setStatus('confirming');
      } catch (err) {
        setStatus('error');
        setResult(r => ({ ...r, errors: ['Erro ao ler o arquivo: ' + err.message] }));
      }
    };
    ...
    <div style={s.page}>
      ...
      {status === 'confirming' && (
        <div style={s.card}>
          ...
          <div style={{ overflowX: 'auto' }}>
              ...
              <tbody>
                {preview.slice(0, 10).map((r, i) => (
                  <tr key={i} style={i % 2 === 0 ? { background: 'rgba(255,255,255,0.02)' } : {}}>
                    ...
                    <td style={{ ...s.td, fontFamily: 'monospace', color: '#9ca3af' }}>{r.cnpj}</td>
                    <td style={s.td}>{r.produto_contratado}</td>
                    <td style={s.td}>{r.cidade} / {r.estado}</td>
                    <td style={s.td}>{r.cartoes_emitidos}</td>
                    <td style={s.td}>R$ {(r.potencial_movimentacao || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</td>
            ...
            {preview.length > 10 && <div style={{ textAlign: 'center', padding: 12, color: '#6b7280', fontSize: '0.8rem' }}>... e mais {preview.length - 10} empresas</div>}
          </div>
        </div>
      )}
      ...
      <div style={s.info}>
        ...
        <ol style={{ paddingLeft: 20, color: '#9ca3af', fontSize: '0.87rem', lineHeight: 2 }}>
          ...
          <li>Confira o preview das empresas reconhecidas</li>
          <li>Clique em "Importar" — empresas já existentes serão atualizadas automaticamente</li>
        </ol>
      </div>
