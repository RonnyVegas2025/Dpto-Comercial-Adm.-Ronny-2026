...
async function resolveIds(rows) {
  ...
  if (novosConsult.size > 0) {
    const { data: criados } = await supabase
      .from('consultores').insert([...novosConsult].map(nome => ({ nome, tipo: 'interno' }))).select('id, nome');
    (criados || []).forEach(c => { consultMap[c.nome.toLowerCase()] = c.id; });
  }
  if (novosParceiros.size > 0) {
    const { data: criados } = await supabase
      .from('parceiros').insert([...novosParceiros].map(nome => ({ nome }))).select('id, nome');
    ...
    try {
      ...
      for (let i = 0; i < rows.length; i += BATCH) {
        ...
        const { data, error } = await supabase
          .from('empresas').upsert(batch, { onConflict: 'produto_id' }).select('id');
    ...
    <div style={s.page}>
      ...
      {status === 'confirming' && (
        <div style={s.card}>
          ...
          <div style={{ overflowX: 'auto' }}>
            ...
            {preview.length > 15 && <div style={{ textAlign: 'center', padding: 12, color: '#6b7280', fontSize: '0.8rem' }}>... e mais {preview.length - 15} empresas</div>}
          </div>
        </div>
      )}
      ...
      <div style={s.info}>
        ...
        <ol style={{ paddingLeft: 20, color: '#9ca3af', fontSize: '0.87rem', lineHeight: 2 }}>
          ...
          <li>Confira o preview — potencial e valores devem aparecer corretamente</li>
