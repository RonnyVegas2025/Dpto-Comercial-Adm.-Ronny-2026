  ...
  async function carregarDash() {
    ...
    try {
      ...
      const { data: viewData, error: viewErr } = await supabase
        .from('vw_agregados_mensal')
        .select('*');

      if (viewErr) {
        console.error('Erro na view vw_agregados_mensal:', viewErr.message);
        // Fallback: busca direto das tabelas sem a view
        const { data: fechData, error: fechErr } = await supabase
          .from('fechamentos_agregados')
          .select(`
            competencia, valor_boleto, custo_mes, lucro_mes, grupo,
            titulares_mes, dependentes_mes,
            contrato:contratos_agregados (
              empresa_agregada_id, is_combo, combo_nome,
              produto_1:produto_1_id (nome),
              empresa:empresa_agregada_id (cnpj, nome,
                consultor_principal:consultor_principal_id (nome)
              )
            )
          `);
        if (fechErr) throw new Error('Fechamentos: ' + fechErr.message);

        rows = (fechData || []).map(f => ({
          competencia:          f.competencia,
          valor_boleto:         f.valor_boleto,
          custo_mes:            f.custo_mes,
          lucro_mes:            f.lucro_mes,
          grupo:                f.grupo,
          total_vidas:          (f.titulares_mes||0) + (f.dependentes_mes||0),
          cnpj:                 f.contrato?.empresa?.cnpj || '',
          empresa:              f.contrato?.empresa?.nome || '—',
          produto_label:        f.contrato?.combo_nome || f.contrato?.produto_1?.nome || '—',
          consultor_principal:  f.contrato?.empresa?.consultor_principal?.nome || '—',
          empresa_agregada_id:  f.contrato?.empresa_agregada_id || '',
        }));
      } else {
        rows = viewData || [];
      }

      if (mesFiltro) rows = rows.filter(r => r.competencia?.substring(0,7) === mesFiltro);

      const ms = [...new Set(rows.map(r => r.competencia?.substring(0,7)).filter(Boolean))].sort();
      setMeses(ms);
      setDados(rows);
    } catch(err) {
      console.error('Erro carregarDash:', err);
      setDados([]);
    }
    setLoadDash(false);
  }
