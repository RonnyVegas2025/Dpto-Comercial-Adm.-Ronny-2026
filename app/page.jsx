      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(280px, 1fr))', gap:16 }}>
        {cards.map(c => (
          <Link key={c.href} href={c.href} style={{ textDecoration:'none' }}>
            <div style={{
              background:'#ffffff', border:'1px solid #e4e7ef', borderRadius:12,
            ...
            >
              <div style={{ display:'flex', alignItems:'flex-start', gap:14 }}>
                ...
                <div>
                  <div style={{ fontWeight:600, fontSize:'0.95rem', color:'#1a1d2e', marginBottom:4 }}>{c.label}</div>
                  <div style={{ fontSize:'0.8rem', color:'#8b92b0', lineHeight:1.4 }}>{c.desc}</div>
                </div>
              </div>
